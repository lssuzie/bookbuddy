...
|:--------|:------------|:-----|:------|
| **3 TTS Modes** | Built-in voices, Voice Design, Voice Clone | 三种 TTS 模式 | 内置音色、声音设计、声音克隆 |
| **8 Built-in Voices** | 4 Chinese + 4 English voices | 8 种内置音色 | 4 中文 + 4 英文 |
| **6 Voice Presets** | Sleep, Meditation, Radio, Gentle, Her, Mystery | 6 种预设 | 睡前催眠、冥想引导、深夜电台、温柔叙述、Her 知性元气、悬疑小说 |
| **Voice Cloning** | Clone any voice with 5s audio | 声音克隆 | 5 秒音频即可克隆 |
| **Smart Chunking** | Auto-adjusts for coherent speech | 智能分片 | 自动调整确保连贯 |
| **Multi-platform** | CLI, Coze Plugin, Dify Tool, GPTs Action | 多平台 | CLI、Coze、Dify、GPTs |
...
bookbuddy/
├── scripts/
│   ├── generate_audio.py    # Main TTS script | 主脚本
│   └── test-bookbuddy.sh    # Test all voice presets | 测试脚本
├── samples/
│   └── 道德经.txt           # Sample text | 示例文本
├── references/
│   ├── skill-voice-guide.md       # Voice recommendations | 音色指南
│   ├── skill-cli-reference.md     # CLI documentation | CLI 文档
│   ├── skill-voice-clone.md       # Voice cloning guide | 克隆指南
│   └── skill-troubleshooting.md   # Troubleshooting | 故障排除
├── coze-plugin/           # Coze Plugin package | Coze 插件包
├── dify-plugin/           # Dify Plugin package | Dify 插件包
├── openai-action/         # OpenAI GPTs Action package | GPTs Action 包
├── examples/
│   └── test-curl.sh       # API test examples | API 测试示例
├── README.md
└── LICENSE
```
...
| Feature | Why It Matters | 特性 | 为什么重要 |
|:--------|:---------------|:-----|:----------|
| **NFKC Normalization** | Fixes OCR/PDF encoding issues | **NFKC 清洗** | 修正 PDF/OCR 的乱码字,读出来不崩 |
| **Smart Chunking** | ≤100 chars for clone, ≤300 for TTS, ≤500 for design | **智能分片** | 防止 TTS 长句读飘、吞音、电音 |
| **Resume Support** | Skip already-generated segments | **断点续传** | 中断了重跑,跳过已生成的,不白等 |
| **Auto Download** | Fetch public domain books automatically | **--download** | 搜公版书/Gutenberg,一键下载转有声书 |
| **Voice Cloning** | Clone any voice with 5s audio | **声音克隆** | 用 5 秒录音克隆任何声线 |
| **Batch Processing** | 30 segments → auto-merge to MP3 | **分卷合并** | 每 30 段自动合并,ffmpeg 成最终 MP3 |
