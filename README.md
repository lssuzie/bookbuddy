...
Samantha 不是 TTS,她是陪伴。我们暂时还造不出一个像 Samantha 那样完整的 AI 伴侣--那种能真正理解你、回应你、和你一起生活的存在。但我们可以从声音开始,一步一步靠近。
...
| 平台 | 安装方式 |
|:-----|:---------|
| **Claude Code** | `/plugin marketplace add lssuzie/bookbuddy` → 选择 `bookbuddy` |
| **Claude.ai** | 平台设置 → 上传 Skill 文件夹 |
| **Claude API** | 通过 Skills API 上传自定义技能 |
